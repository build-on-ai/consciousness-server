...
only valid requests pass; invalid → `401`, and an unreachable key-server
→ `503`.

Key-server itself has no opinion about what blocks do with its verdict; it
only answers whether the signature checks out.
...
## Side effects of a verify call

Key-server writes two things:

- `SET ks:nonce_seen:<nonce> 1 NX EX 300` — Redis, for anti-replay.
  The nonce is claimed atomically **during** verification (after the
  timestamp check, before the signature check), so it is consumed
  even when the request is ultimately rejected — not only on
  success. See Anti-replay above.
- One line to `logs/audit.jsonl` — `{ts, event:"verify",
  agent_id, method, path, result, ip}`, written for both passing and
  failing verifications. Key-server self-rotates the file at 50 MB.
...
## In-process signer for Node blocks

Blocks in this repo that call a gated endpoint on a sister service
sign with `lib/sign-outbound.js` (`signHeaders(agentId, method,
path, body)` → the four `X-*` headers, or `{}` when
`CS_SIGNING_KEY` is unset). Note the same OpenSSH
key caveat as `bin/sign-request`: Node's `crypto.createPrivateKey`
does not read OpenSSH ed25519 containers, so the module parses the
key with `sshpk` and re-wraps it as PKCS8.
...
This is a candidate for a future protocol version. What it exposes, and
where, is written up in `SECURITY.md` ("Query string is outside the
signature"). Extending the canonical format means changing the Node
verifier in `lib/`, the Go signer and key-server together.
...
outage flip blocks to `observe` — a per-block compose restart, about
10 seconds each.
