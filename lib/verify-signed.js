...
// Discovery is read before a client has an identity, so reading it needs no signature.
// Only reading: anything that would change it is still gated.
const PUBLIC_READ_PATHS = new Set(['/.well-known/agent.json']);
...
function shouldSkip(method, reqPath, headers) {
  ...
  if (PUBLIC_READ_PATHS.has(reqPath) && PUBLIC_READ_METHODS.has(verb)) return true;
...
function reinjectBody(req, bodyBytes) {
  ...
  if (isJson) {
    try {
      ...
      // The body is parsed here, so a broken one reaches handlers as an empty object.
      // The failure is recorded; what a route does with it is the route's call.
      req.body = {};
