#!/usr/bin/env python3
"""Semantic search over stored notes: ChromaDB for storage, Ollama for embeddings."""
...
from middleware.ports import own_port
...
PORT = own_port('semantic-search', 3037)
...
def get_embedding(text: str):
    ...
    Returns (embedding, None) or (None, {'reason', 'fix'}) so the caller can answer 503.
    """
    try:
...
@app.route('/api/notes/<doc_id>/preview', methods=['GET'])
def note_preview(doc_id):
    """Full note content plus hit ranges.

    Returns ranges, not markup: the caller owns escaping.
    """
    query = request.args.get('q')
    try:
        window = int(request.args.get('window', 400))
    except ValueError:
        window = 400

    collection_name = request.args.get('collection', 'notes')
    ...
    text = (got.get('documents') or [''])[0] or ''

    result = preview(text, query, window=window)
    result.update({
        'id': doc_id,
        'title': meta.get('title') or meta.get('topic') or '',
        'created_at': meta.get('created_at'),
        'author': meta.get('agent'),
