...
- `query` — free-text search string. Be specific: "redis connection
  pool sizing" beats "database".
...
- If multiple hits contradict each other, surface that explicitly:
  "memory gives two answers, recorded three months apart". Let the
  human decide which is current.
