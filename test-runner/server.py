...
"""Test runner service: runs a project's test suite and reports the result."""
...
PORT = own_port('test-runner', 3041)

app = Flask(__name__)
CORS(app)
flask_middleware(app)  # every request must carry a valid ed25519 signature
...
@app.route("/health")
def health():
    return jsonify({"status": "ok", "service": "test-runner", "port": PORT})
